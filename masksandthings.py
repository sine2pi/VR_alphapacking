...
def antialiased_upscale(img, H, W, scale_factor=2):
    large_w = int(W * scale_factor)
    large_h = int(H * scale_factor)
    large_img = torch.nn.functional.interpolate(
                            mask_expanded, size=(large_h, large_w), mode='bicubic', align_corners=False, antialias=True
                        )
    blurred = torchvision.transforms.functional.gaussian_blur(large_img, kernel_size=3)
    final_img = torch.nn.functional.interpolate(
                            blurred, size=(H, W), mode='bicubic', align_corners=False, antialias=True
                        )
    return final_img
...
def mask_edges(mask: torch.Tensor, kernel_size: int = 1) -> torch.Tensor:
    ...
    return (x).view(orig_shape).float()
    # return (x > 0.5).view(orig_shape).float()
...
def VFG(video, force_rate=0, frame_load_cap=0, skip_first_frames=0, select_every_nth=1, start_time=0.0, output_format="tensor", **kwargs):
    try:
...
class RaftMotionCompensator:
    ...
    def _compute_raft_flow(self, img1, img2):
        ...
        # Disable any outer autocast (e.g. bfloat16 from SAM3 loop) — RAFT weights are float32
        with torch.autocast(device_type=self.device.type, enabled=False):
            flow = self.model(img1_t.to(self.device).float(), img2_t.to(self.device).float())[-1].float()
    ...
    def interpolate_video(self, input_video, output_video, src_fps, dst_fps, width, height, encoder_opts, read_cmd=None):
        ...
        buf_size = min(width * height * 3 * 2, 10 ** 8)
        reader = subprocess.Popen(read_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, bufsize=buf_size)
        writer = subprocess.Popen(write_cmd, stdin=subprocess.PIPE, bufsize=buf_size)
