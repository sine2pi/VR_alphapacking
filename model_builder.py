...
def mask_edges(mask: torch.Tensor, kernel_size: int = 1) -> torch.Tensor:
    ...
    x = V.gaussian_blur(x, kernel_size=blur_k, sigma=float(blur_radius))
    x = (x > 0.5).to(mask.dtype)
    return x.view(orig_shape)

def apply_effects(masks, dilation, feather_radius, smooth_edges, smoother_edges: int = 0, fill_holes: bool = False, mask_blur: int = 0, mask_offset: int = 0, invert_output: bool = False, kernel_size: int = 3):
    ...
    for mask in masks:
        ...
        if isinstance(mask, np.ndarray):
            mask = torch.from_numpy(mask).to(device).float() / 255.0
...
def process_frames(predictor, raft, frames_pil, frames_bgr, prompt_text, prior_mask=None, warp=False):
...
def process_videos(video_path, out_path, out_mask_path, prompt_text, batch_size, matte_size, warp):  
    ...
    while frame_count < frames:
        ...
        masks_l = apply_effects(masks_l, dilation=0, feather_radius=1.2, smooth_edges=0, smoother_edges=0, fill_holes=False, mask_blur=0, mask_offset=0, invert_output=False)
        masks_r = apply_effects(masks_r, dilation=0, feather_radius=1.2, smooth_edges=0, smoother_edges=0, fill_holes=False, mask_blur=0, mask_offset=0, invert_output=False)
